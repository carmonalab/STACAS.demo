---
title: "Generate synthetic scRNA-seq data with splatter"
author: "S. Carmona"
output:
...
The following function will generate synthetic datasets, in two possible modes:

* mode `twoGroups=FALSE`: will generate 2 batches with 1 cell type

* mode `twoGroups=TRUE`: will generate 3 batches including two cell types/groups:
...
Batch 1 contains a frequency of `freqBatchAB` of total cells; and the rest are equally divided between Batches 2 and 3.

```{r}
simulateAndMeasure <- function(ncellsTotal, freqBatchAB, freqBatchA=NULL,
                               twoGroups=T, de.facLoc=0.2, de.facScale=0.2,
                               batch.facScale=0, batch.facLoc=0, mySeed=123,
                               k=20, cell_min=10, batch_min=NULL, ncores=8){
...
if (twoGroups){
  ...
  test <- splatSimulate(batchCells = batchCells, group.prob = c(group.prob, 1-group.prob),
                        method = "groups", verbose = FALSE, de.facLoc=de.facLoc,
                        de.facScale=de.facScale, batch.facScale= batch.facScale,
                        batch.facLoc=batch.facLoc ) # see ?SplatParams  de.facLoc=0.2, de.facScale=0.2
  ...
  test <- splatSimulate(batchCells = batchCells, verbose = FALSE,
                        de.facLoc=de.facLoc, de.facScale=de.facScale,
                        batch.facScale= batch.facScale, batch.facLoc=batch.facLoc )
  test$Group <- "Group1"
}
...
test <- evalIntegration(test, metrics = c("isi", "cms"),
                        group = "Batch", k = k, n_dim = 2,
                        cell_min = cell_min, weight = F,
                        batch_min = batch_min,
                        BPPARAM=MulticoreParam(workers=ncores))

if(twoGroups){
test <- evalIntegration(test, metrics = c("isi", "cms"),
                        group = "Group", k = k, n_dim = 2,
                        cell_min = cell_min, weight = F, batch_min = batch_min,
                        BPPARAM=MulticoreParam(workers=ncores),
                        res_name = c("cluster_lisi", "cluster_cms"))
}
...
plotSimul <- function(test){
  lab <- sprintf("cms %.2f; lisi %.2f; cluster_cms %.2f; cluster_lisi %.2f",
                 mean(test$cms_smooth),mean(test$isi),
                 mean(test$cms_smooth.cluster_cms),
                 mean(test$cluster_lisi))
  a <- plotPCA(test, shape_by = "Group", colour_by = "Batch") + 
    scale_color_discrete(labels = paste(levels(test$Batch),":",table(test$Batch))) + 
    coord_fixed() + 
    labs(subtitle = lab)
  b <- plotPCA(test, shape_by = "Batch", colour_by = "Group") + coord_fixed()
  a | b
}
```
...
for (batch.factor in seq(0,0.12,by=0.03)) { 
 name <- paste0("batch0_single_batchFactor_",batch.factor)
  test.list.single[[name]] <- simulateAndMeasure(ncellsTotal = ncellsTotal,
                                                 freqBatchAB = freqBatchAB,
                                                 batch.facScale = batch.factor,
                                                 batch.facLoc = batch.factor,
                                                 k=k, cell_min=cell_min,
                                                 ncores=ncores, twoGroups = F)
}
```
...
pp <- lapply(names(test.list.single), function(name) { 
  labs <- paste(levels(test.list.single[[name]]$Batch),":",
                table(test.list.single[[name]]$Batch))
  title <- sprintf("cms %.2f; lisi %.2f",
                   mean(test.list.single[[name]]$cms_smooth),
                   mean(test.list.single[[name]]$isi))
  
  a <- plotPCA(test.list.single[[name]], shape_by = "Group", colour_by = "Batch") +
    scale_color_discrete(labels = labs) + coord_fixed() +
    labs(subtitle = title) & ggtitle(name) & theme(aspect.ratio=1)
  b <- visHist(test.list.single[[name]],metric = c("cms"))
  a | b
  })
pp
...
test.list.single.metrics %>% pivot_longer(cols=c("cms","lisi")) %>%
  ggplot(aes(x=batchFactor, y=value, group=name)) + geom_line() +
...
for (myfreqBatchAB in seq(0.2,0.8,by=0.1)) { 
 name <- paste0("batch0_single_balance_",myfreqBatchAB)
  test.list.single.balance[[name]] <- simulateAndMeasure(ncellsTotal = ncellsTotal,
                                                         freqBatchAB = myfreqBatchAB,
                                                         batch.facScale = 0, batch.facLoc = 0,
                                                         k=k, cell_min=cell_min,
                                                         ncores=ncores, twoGroups = F)
}
```
...
pp <- lapply(names(test.list.single.balance), function(name) {
  labs <- paste(levels(test.list.single.balance[[name]]$Batch),":",
                table(test.list.single.balance[[name]]$Batch))
  title <- sprintf("cms %.2f; lisi %.2f",
                   mean(test.list.single.balance[[name]]$cms_smooth),
                   mean(test.list.single.balance[[name]]$isi))
  
  a <- plotPCA(test.list.single.balance[[name]],
               shape_by = "Group", colour_by = "Batch") + 
    scale_color_discrete(labels = labs) + coord_fixed() + 
    labs(subtitle = title) & ggtitle(name) & theme(aspect.ratio=1)
  b <- visHist(test.list.single.balance[[name]],metric = c("cms"))
...
test.list.single.balance.metrics.balance %>% 
  pivot_longer(cols=c("cms","lisi")) %>%
  ggplot(aes(x=balance, y=value, group=name)) + 
...
```{r}
test.list[["batchStrong"]] <- simulateAndMeasure(ncellsTotal = ncellsTotal,
                                                 freqBatchAB = freqBatchAB,
                                                 batch.facScale = batch.factor.strong,
                                                 batch.facLoc = batch.factor.strong,
                                                 k=k, cell_min=cell_min, ncores=ncores)
plotSimul(test.list[["batchStrong"]])
```
Here the batch effects are large, and cells are separately by batch and cell type.
...
```{r}
test.list[["batchMild"]] <- simulateAndMeasure(ncellsTotal = ncellsTotal,
                                               freqBatchAB = freqBatchAB,
                                               batch.facScale = batch.factor.small,
                                               batch.facLoc = batch.factor.small, k=k,
                                               cell_min=cell_min, ncores=ncores)
...
```{r}
test.list[["batch0"]] <- simulateAndMeasure(ncellsTotal = ncellsTotal,
                                            freqBatchAB = freqBatchAB,
                                            batch.facScale = 0,
                                            batch.facLoc = 0, k=k,
                                            cell_min=cell_min, ncores=ncores)
...
```{r}
test.list[["batch0_unbalanced"]] <- simulateAndMeasure(ncellsTotal = ncellsTotal,
                                                       freqBatchAB = freqBatchAB/2,
                                                       batch.facScale = 0, batch.facLoc = 0,
                                                       k=k, cell_min=cell_min, ncores=ncores)
plotSimul(test.list[["batch0_unbalanced"]])
```
When number of data points in the batches is not balanced, the cms metric drops dramatically, even though there is zero batch effect. Lisi decreases as well, but less dramatically. It's good to keep in mind that these metrics may not be sensitive to differential dataset size.


```{r}
test.list[["overcorrected"]] <- simulateAndMeasure(ncellsTotal = ncellsTotal,
                                                   freqBatchAB = freqBatchAB,
                                                   batch.facScale = 0, batch.facLoc = 0,
                                                   de.facLoc = 0, de.facScale = 0, k=k,
                                                   cell_min=cell_min, ncores=ncores)
...
On the other hands, because cells of different type mix with each other, cluster_cms and cluster_lisi strongly increase (=bad cluster separation).
...
plot.list <- lapply(names(test.list),function(name){
  a <- plotSimul(test.list[[name]]) & ggtitle(name) & theme(aspect.ratio=1) 
  b <- visHist(test.list[[name]],metric = c("cms"))
  a | b
})
...
Export data sets as Seurat objects to use them in other notebooks
```{r, eval=F, message=F, warning=F,results=F}
ndim=2

exportLists <- list(batchEffect=test.list.single,
                    batchBalance=test.list.single.balance,
                    batchCellTypes=test.list)
