...
p2_pre <- DimPlot(object, group.by = "final_annotation", label=T, label.size = 4) +
...
# perplexity=effective number of each cell's neighbors; use lower than default to make it faster
integrationMetrics[["uncorrected"]][["batch_LISI"]] <- mean(compute_lisi(object@reductions[["pca"]]@cell.embeddings,meta_data = object@meta.data, label_colnames="batch", perplexity = lisi_perplexity)[[1]]) 
...
p2_int <- DimPlot(object_integrated, group.by = "final_annotation", label=T, label.size = 2) +
