...
DimPlot(object.uncorrected, group.by="scGate_multi") + theme(aspect.ratio = 1)

ggsave("CD4_CD8_scGate_uncorrected_space.png", height=4.5, width=5)

```

Here we indicate in `cell.labels` the metadata column that contains cell annotations. We can generate simple annotations using e.g. scGate:

```{r, results=F, warning=F, message=F }
object_integrated_ss <- obj.list %>% Run.STACAS(dims = 1:ndim, anchor.features = hvg, cell.labels = "scGate_multi")
```
...
```{r, results=F, warning=F, message=F }
object_integrated_ss_full <- obj.list %>% Run.STACAS(dims = 1:ndim, anchor.features = hvg, cell.labels = meta.label)
```
...
Visualize on UMAP space
```{r results=F, warning=F, message=F}
object_integrated_ss_full <- object_integrated_ss_full %>% RunUMAP(dims=1:ndim)
```

```{r, results=F, warning=F, message=F}
method = "semisupSTACAS_full"
method.reduction <- "pca"
...
integrationMetrics[[method]] <- getIntegrationMetrics(object=metricsObject,
                                                      ...
                                                      method.reduction = method.reduction, metricsLabels = metricsLabels)
...
```{r, results=F, warning=F, message=F, fig.height=8, fig.width=12, eval=T}
...
ggsave("STACAS.compareMetrics.Tcells.summary.png", width = 16, height = 14)
```

```{r fig.height=3, fig.width=14}
a <- integrationMetricsSummary %>% filter(Metric %in% c("celltype_ASW_means","batch_nLISI_perCellType_means")) %>%
...
ggsave("STACAS.compareMetrics.Tcells.scatter.png", plot = b, width = 5.5, height = 3.5)
ggsave("STACAS.compareMetrics.Tcells.scatter.pdf", plot = b, width = 5.5, height = 3.5)
```
