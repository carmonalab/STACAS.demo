...
knit: (function(input_file, encoding) {
 ...
 output_file=file.path(dirname(input_file), out_dir, 'test_CellMixS_batchmin.html'))})
---
...
With the wrapper functions defined above, we will  evaluate a number of metrics implemented in the CellMixS Bioconductor package
...
In the initial configuration (k=20 and default cms parameters), mean cms and lisi peak at equal batch size. Both metrics are strongly affected by batch size unbalance. The larger the unbalance, the lower cms/lisi.
Here we try if other cms configurations make it robust to unbalance (using batch_min and increased k) while preserving other desirable properties
