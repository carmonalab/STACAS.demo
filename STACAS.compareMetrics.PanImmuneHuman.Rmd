...
g <- ggplot(integrationMetricsSummary, aes(x=reorder_within(Method,-value, Metric),
...
g
ggsave("plots/panImmune_metrics.pdf", g, width = 8, height=18)

```

```{r}
integrationMetricsSummary %>% filter(Metric %in% c("celltype_ASW_means","batch_nLISI_perCellType_means")) %>% pivot_wider(names_from = Metric, values_from = value ) %>% ggplot(aes(x=batch_nLISI_perCellType_means, y=celltype_ASW_means, label=Method)) + geom_point(aes(color=Method))  + geom_text(hjust=0.3, vjust=0.2) +  theme_light()


ggsave("plots/panImmune_metrics.scatter.pdf", width = 6, height=4)
```
