...
To evaluate integration quality we use different metrics that consider two aspects of integration: i) batch mixing of cells, and ii) conservation of biological variance of cell identity.
For i) because batch-mixing of cells of different type is not desirable, we include a variation of LISI ("per celltype iLISI" or ciLISI) to consider only cells of the same cell type at a time.
For ii) we use the average silhouette width (AWS; aka silhouette coefficient) of the corrected PCA space, that measures the relationship between the within-cluster (i.e. intra-celltype) distances of a cell and the between-cluster (i.e. inter-celltype) distances of that cell to the closest cluster/cell type.
...
Let's quantify dataset/batch mixing using the [LISI metric](https://github.com/immunogenomics/LISI) and conservation of biological information by means of Average Silhouette Width (ASW) of cell labels in the corrected PCA space:
