...
```{r}
integrationMetricsSummary %>% filter(Metric %in% c("celltype_ASW","batch_nLISI_perCellType")) %>% pivot_wider(names_from = Metric, values_from = value ) %>% ggplot(aes(x=batch_nLISI_perCellType, y=celltype_ASW, label=Method)) + geom_point(aes(color=Method))  + geom_text(hjust=0.1, vjust=0) +  theme_light()
```
