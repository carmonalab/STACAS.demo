...
NOTE: The calculation of highly variable genes (HVG) is a fundamental step for dimensionality reduction (and for integration based on low dimensional representations). We recommend excluding certain classes of genes, e.g. mitochondrial, ribosomal, heat-shock genes, from the list of variable genes, as their expression may be more related to technical variability rather than to cell type identity. Similarly, VDJ genes are highly individual-specific. FindVariableFeatures.STACAS function (default STACAS' behavior) calculates HVG excluding non-desirable gene sets:
...
```{r, results=F, warning=F, message=F, fig.height=8, fig.width=16, eval=F }
