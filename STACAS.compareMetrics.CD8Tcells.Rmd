...
compute_lisi_splitByClass <- function(object, method.reduction, meta.batch, meta.label, labels.use=NULL, minCellPercPerClass=5, ...){
  ...
  for ( i in seq_len(length(object.list))){
    ...
    batchNum <- length(t)
    cellPerBatchEqual <- 100/batchNum
    #message(paste(this.class,round(t.norm,2)," "))
    
    batch.use.here <- names(t[t.norm > min(minCellPercPerClass,cellPerBatchEqual)]) # which batches contribute with at least `minCellPercPerClass` or the expected number of cells if all batches were of same size?
