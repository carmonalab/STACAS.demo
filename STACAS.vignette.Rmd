---
title: Integrating scRNA-seq data using STACAS and Seurat 3
author: 
...
For installation of the STACAS package refer to the [STACAS Github repository](https://github.com/carmonalab/STACAS)
...
library(grid)
library(gridExtra)
cellCycle.symbol <- read.csv("aux/cellCycle.symbol.DE.specific.170120.csv",as.is=T)$x
```
...
DefaultAssay(ref.integrated.default) <- "integrated"
##Clean up memory
rm(list = c('ref.integrated.default','ref.anchors.default','ref.list.default'))
```
...
Downsample and export object
```{r}
ref.integrated.diet <- DietSeurat(ref.integrated, counts=FALSE,data=TRUE, scale.data =FALSE, dimreducs = c("pca","umap"))
Idents(ref.integrated.diet) <- "celltype.pred.states"
set.seed(1234)
ref.integrated.diet <- subset(ref.integrated.diet, cells = WhichCells(object = ref.integrated.diet, idents = levels(Idents(ref.integrated.diet)), downsample = 1000) )

saveRDS(ref.integrated.diet, file="stacas.cd4cd8.integrated.rds")
```
